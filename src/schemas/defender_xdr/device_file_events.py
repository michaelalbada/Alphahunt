...
class DeviceFileEvents(Base):  
    __tablename__ = 'DeviceFileEvents'  
    ...
    FileOriginReferrerUrl = Column(
        String(1024),
        comment="URL of the web page that links to the downloaded file"  
    )
    ...
    PreviousFolderPath = Column(
        String(1024),
        comment="Original folder containing the file before the recorded action was applied"  
    )

    PreviousFileName = Column(
        String(255),
        comment="Original name of the file that was renamed as a result of the action"  
    )
    ...
    InitiatingProcessFileSize = Column(
        BigInteger,
        comment="Size of the process (image file) that initiated the event"  
    )
    ...
    RequestProtocol = Column(
        String(50),
        comment="Network protocol, if applicable, used to initiate the activity: Unknown, Local, SMB, or NFS"  
    )

    RequestSourceIP = Column(
        String(45),
        comment="IPv4 or IPv6 address of the remote device that initiated the activity"  
    )

    RequestSourcePort = Column(
        Integer,
        comment="Source port on the remote device that initiated the activity"  
    )

    RequestAccountName = Column(
        String(255),
        comment="User name of account used to remotely initiate the activity"  
    )

    RequestAccountDomain = Column(
        String(255),
        comment="Domain of the account used to remotely initiate the activity"  
    )

    RequestAccountSid = Column(
        String(128),
        comment="Security Identifier (SID) of the account used to remotely initiate the activity"  
    )

    ShareName = Column(
        String(255),
        comment="Name of shared folder containing the file"  
    )

    SensitivityLabel = Column(
        String(255),
        comment="Label applied to an email, file, or other content to classify it for information protection"  
    )

    SensitivitySubLabel = Column(
        String(255),
        comment="Sublabel applied to an email, file, or other content to classify it for information protection; sensitivity sublabels are grouped under sensitivity labels but are treated independently"  
    )

    IsAzureInfoProtectionApplied = Column(
        Boolean,
        comment="Indicates whether the file is encrypted by Azure Information Protection"  
    )
